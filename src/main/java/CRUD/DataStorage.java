...
public class DataStorage{
     ...
     */
    public void createData(String data, String name){
        dataStorage.add(new DataClass(data, name));
     ...
     */
    public void updateData(String data2, String name){
        int index = -1;
        for (DataClass data : dataStorage) {
            if(data.getName().equals(name)){
    ...
    public String sendDataSerial(){
        try {
            ...
            ByteArrayOutputStream bo = new ByteArrayOutputStream();
            ObjectOutputStream so = new ObjectOutputStream(bo);
            // for (DataClass dataClass : dataStorage) {
            //     System.out.println("name: "+dataClass.getName());
            //     System.out.println("Entry: "+dataClass.read());
            // }
            so.writeObject(dataStorage);
            so.flush();
            String code = new String(Base64.getEncoder().encode(bo.toByteArray()));
